...
注意：
- cleanup_class 判定规则（严格执行）：
  · cleanup: 手机截图、聊天记录截图、纯文字图片、App 界面截图、快递单、二维码、模糊/过曝到无法辨认的照片、重复的连拍（burst）、纯黑/纯白废片
  · review: 内容不确定是否有保留价值的照片（如：拍到一半的、构图很差但可能有纪念意义的）
  · keep: 有明确场景/人物/风景/纪念意义的照片
  · 如果上下文标记了 is_screenshot=true，默认应为 cleanup 除非内容确实重要
