...
- 不做 Apple 已经做好的事（物体标签、人脸检测、基础关键词搜索）
- 不做创作者素材管理工具（不同产品、不同用户）
- 不依赖云端服务
- 不在 AppleScript 写回能力上建立核心价值（平台风险）

## Three Usage Scenarios (added after /autoplan CEO review)

L2 的结构化数据同时驱动三个场景，不只押注叙事：

1. **智能搜索** — "找我去年和李四在云南的照片"、"最近拍的有文字的截图"
2. **智能清理** — "帮我标记可以删的模糊照片和重复截图"
3. **回忆叙事** — "生成和女友的回忆时间线"
...
**保留用于搜索/清理的字段：**
```python
    "search_tags": ["聚餐", "海边", "合影"],  # 简洁的可搜索场景标签
    "has_text": True,
    "text_summary": "如果有文字，提取关键内容",
    "cleanup_class": "keep",  # keep | review | cleanup（截图/模糊/重复）
    "duplicate_hint": "burst",  # burst | live_variant | edited | standalone
```

**不再输出：** colors, quality_notes, objects, animals, location_type, time_of_day（Apple 元数据已覆盖或无用）
...
→ 发现 3 位高频出现但未命名的人物：
  ...
  运行 phototag people --name face_002 "李四" 来命名
```
...
# 关系回忆线
phototag story --relationship "李四"
→ 输出 Markdown：
  # 和李四在一起的日子
  从 2023 年 1 月到现在，共 500 天...

# 导出为文件
phototag story --person "李四" --output story.md
phototag story --person "李四" --output story.json
```
...
- `phototag generate --template "couple" --person "李四" --output story.html`
- 精美 HTML 模板，内嵌照片缩略图
- 可导出 PDF
- 订阅制：每月自动生成回忆邮件
...
## Milestones (revised after /autoplan review)

| Milestone | Scope | Success Criteria | Est. Effort |
|-----------|-------|------------------|-------------|
| **M1.5** | Model eval gate: 用 50 张照片手动测试本地模型的叙事+搜索+OCR 质量上限 | narrative 准确率 >= 70%, 搜索标签有用率 >= 80%, 截图 OCR 可读 | ~0.5 day |
| **M2** | Layer 1 重构: Apple 元数据 + 新 prompt + 搜索+叙事双输出 + AppleScript sanitize + DB 迁移 + reprocess 命令 | 20 张照片验证通过 | ~1 week |
| **M2.5** | Dedup 前置: 在事件聚合前去重/burst 抑制 | burst/Live Photo 不污染后续统计 | ~2 days |
| **M3** | Layer 2 事件聚合: 时间+人脸+来源多信号 | 20 个事件 80% 边界合理 | ~1 week |
| **M4** | Layer 2 人物图谱: 统计 + 关系推断 + 命名引导 | 核心人物全部识别 | ~1 week |
| **M5** | Layer 3 叙事+搜索+清理: `phototag story/search/cleanup` | **自己用了一周不想删** | ~1 week |
| **M6** | 社交验证: 发布到社交媒体 | > 5 人问"这怎么做的" | ~1 day |

**M1.5 是第一个 kill gate。** 如果本地模型天花板太低，立刻决定：换大模型、接 API、或调整方向。
**M5 是第二个 kill gate。** 验证标准从"想发朋友圈"改为"自己用了一周不想删"（留存 > 分享）。
...
## Implementation Order (revised after /autoplan eng review)

### M1.5: Model Eval Gate (first)

手动测 50 张照片（含人像、风景、截图、文档、视频帧），验证本地模型的质量天花板。
不写代码，直接用脚本调 Ollama API 测试新 prompt，人工评分。

### M2: Layer 1 重构

1. **db.py** — 加 schema version 表 + ALTER TABLE 迁移逻辑
2. **scanner.py** — 采集 Apple 元数据（labels, face_cluster_ids, named_faces, source_app, selfie, screenshot, place info）
3. **recognizer.py** — 重写 prompt，传入 photo_context，输出 narrative + search_tags + OCR + cleanup_class
4. **tagger.py** — sanitize 所有拼入 AppleScript 的字符串（修复注入漏洞）
5. **processor.py** — 串联 scanner 元数据 → recognizer
6. **cli.py** — `run` 默认最近 6 个月 + `--all`；加 `reprocess` 命令

### M2.5: Dedup 前置

将 dedup 集成到 process_batch 流程中，burst/Live Photo 在事件聚合前抑制。
...
|--------|---------|------|--------|-------------|
| CEO Review | /autoplan | 1 | issues_open | 叙事方向过窄→加搜索+清理；核心前提未验证 |
| CEO Voices | dual | 1 | 4/6 confirmed | 两个模型一致：先验证模型质量再建架构 |
| Eng Review | /autoplan | 1 | issues_open | AppleScript 注入、DB 迁移缺失、无 reprocess、里程碑重排 |
| Eng Voices | dual | 1 | 5/6 confirmed | 两个模型一致：保留搜索字段、dedup 前置 |

**VERDICT:** APPROVED with revisions — 搜索+清理场景已加入，里程碑已重排，安全+迁移问题已纳入 M2。
